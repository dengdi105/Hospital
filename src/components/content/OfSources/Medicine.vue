        ...
        <div class="header">
            <el-form :inline="true">
                ...
                <el-form-item label="药品类别" prop="type">
                    <el-input v-model="search_type" type="text" placeholder="请输入药品类别"></el-input>
                </el-form-item>
...
<!--                </el-table-column>-->
                ...
                >
                    <template slot-scope="scope">
                        <el-tag size="medium">{{ scope.row.drug_kind }}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="drug_type"
                ...
                >
                    <template slot-scope="scope">
                        <el-tag type="warning" size="medium">{{ scope.row.drug_type }}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column
                        prop="drug_brand"
...
<!--        修改框-->
        <el-dialog title="药物信息" :visible.sync="dialogFormVisible2" width="45%" top="5vh">
            <el-form :model="resetForm" :inline="true" :rules="rules" ref="resetForm">
                <el-form-item label="药物名称" label-width="70px">
                    <el-input v-model="resetForm.drugName" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="品牌" label-width="50px" class="form-right">
                    <el-input v-model="resetForm.drug_brand" type="text" :rows='3' autocomplete="off"></el-input>
                </el-form-item>
                <br/>
                <br/>
                <el-form-item label="价格" label-width="50px">
                    <el-input-number v-model="resetForm.drug_price" @change="handleChangePrice" :min="1" :max="1000"
                                     :precision="2"></el-input-number>
                </el-form-item>
                <el-form-item label="数量" label-width="50px" class="form-right">
                    <el-input-number v-model="resetForm.drug_num" @change="handleChangeNum" :min="1"
                                     :max="100000"></el-input-number>
                </el-form-item>
                <br/>
                <el-form-item label="剂型" label-width="50px">
                    <el-select v-model="resetForm.drug_kind" placeholder="请选择">
                        <el-option label="颗粒" value="颗粒"></el-option>
                        <el-option label="溶液" value="溶液"></el-option>
                        <el-option label="喷雾" value="喷雾"></el-option>
                    </el-select>
                ...
                <el-form-item label="类别" label-width="50px" class="form-right">
                    <el-select v-model="resetForm.drug_type" placeholder="请选择">
                        <el-option label="口服" value="口服"></el-option>
                        <el-option label="外用" value="外用"></el-option>
                    </el-select>
                </el-form-item>
                <br/>
                <el-form-item label="生产日期" label-width="70px">
                    <el-date-picker v-model="resetForm.drug_ctime" type="datetime"
                                    placeholder="选择日期时间"></el-date-picker>
                </el-form-item>
                <el-form-item label="过期时间" label-width="7 0px" class="form-right">
                    <el-date-picker v-model="resetForm.drug_etime" type="datetime"
                                    placeholder="选择日期时间"></el-date-picker>
                </el-form-item>
                <br/>
                <el-form-item label="厂商名称" label-width="70px">
                    <el-input v-model="resetForm.drug_cname" type="text" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="厂商地址" label-width="70px" class="form-right">
                    <el-input v-model="resetForm.drug_address" type="text" autocomplete="off"></el-input>
                </el-form-item>
                <el-form-item label="入库时间" label-width="70px" >
                    <el-input v-model="resetForm.in_time" type="text" autocomplete="off" disabled></el-input>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="resetDrugs('resetForm')">修改</el-button>
    ...
    export default {
        ...
        methods: {
            dateFormat:function(row){
                let t=new Date(row.drug_ctime);//row 表示一行数据, updateTime 表示要格式化的字段名称
            ...
            dateFormat2:function(row){
                let t=new Date(row.drug_etime);//row 表示一行数据, updateTime 表示要格式化的字段名称
                let year=t.getFullYear(),
                    month=t.getMonth()+1,
                    day=t.getDate();
                let newTime=year+'-'+
            ...
            handleEdit(row){
                ...
                this.resetForm.drug_name = row.drug_name
                this.resetForm.drug_kind = row.drug_kind
                this.resetForm.drug_type = row.drug_type
                this.resetForm.drug_brand = row.drug_brand
                this.resetForm.drug_ctime = row.drug_ctime
                this.resetForm.drug_etime = row.drug_etime
                this.resetForm.drug_cname = row.drug_cname
                this.resetForm.drug_address = row.drug_address
                this.resetForm.drug_price = row.drug_price
                this.resetForm.drug_num = row.drug_num
                this.resetForm.in_time = row.in_time
            },
            handleDelete(row){  //删除
                const _this = this
            ...
            resetDrugs(formName){
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        this.$axios.put('http://localhost:8181/drugs/update',this.resetForm).then(function (resp) {
                            console.log(resp)
            ...
            page(currentPage){
                // alert(currentPage)
                const _this = this
                this.$axios.get('http://localhost:8181/drugs/findAll/'+currentPage+'/8').then(function (resp) {
